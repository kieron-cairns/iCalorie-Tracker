...
class CalorieItemListViewModelTests: XCTestCase {
    ...
    func test_save_with_valid_data() {
        let id = UUID()
        sut.saveCalorieItem(title: "TestTitle", id: id, calorieCount: 100, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
    ...
    func test_save_with_invalid_data() {
        ...
        let resultNilTitle = sut.saveCalorieItem(title: nil, id: UUID(), calorieCount: 100, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        XCTAssertFalse(resultNilTitle.success)
        XCTAssertTrue(resultNilTitle.message.contains("The title cannot be nil."))

        let resultEmptyTitle = sut.saveCalorieItem(title: "", id: UUID(), calorieCount: 100, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultNilCalorieCount = sut.saveCalorieItem(title: "ValidTitle", id: UUID(), calorieCount: nil, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultZeroCalorieCount = sut.saveCalorieItem(title: "ValidTitle", id: UUID(), calorieCount: 0, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultNegativeCalorieCount = sut.saveCalorieItem(title: "ValidTitle", id: UUID(), calorieCount: -100, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultNilTitleAndNilCalorieCount = sut.saveCalorieItem(title: nil, id: UUID(), calorieCount: nil, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultEmptyTitleAndZeroCalorieCount = sut.saveCalorieItem(title: "", id: UUID(), calorieCount: 0, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        ...
        let resultNilTitleAndNegativeCalorieCount = sut.saveCalorieItem(title: nil, id: UUID(), calorieCount: -100, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
    ...
    func test_update_with_invalid_data() {
        ...
        let validId = UUID()
        sut.saveCalorieItem(title: "InitialValidTitle", id: validId, calorieCount: 123, caloireQuantity: 1, date: mockDate, viewContext: mockContext)
        
        // 1. Test with a nil title
        var result = sut.updateCalorieItem(withId: validId, title: nil, calorieCount: 100, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("The title cannot be nil."))
        
        // 2. Test with an empty title
        result = sut.updateCalorieItem(withId: validId, title: "", calorieCount: 100, calorieQuantity: 1,   viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("The title cannot be empty."))

        // 3. Test with a nil calorieCount
        result = sut.updateCalorieItem(withId: validId, title: "ValidTitle", calorieCount: nil, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("Calorie count cannot be nil."))

        // 4. Test with a zero calorieCount
        result = sut.updateCalorieItem(withId: validId, title: "ValidTitle", calorieCount: 0, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("Calorie count must be greater than zero."))

        // 5. Test with a negative calorieCount
        result = sut.updateCalorieItem(withId: validId, title: "ValidTitle", calorieCount: -100, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("Calorie count must be greater than zero."))

        // 6. Test with a nil title and nil calorieCount
        result = sut.updateCalorieItem(withId: validId, title: nil, calorieCount: nil, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("The title cannot be nil."))
        XCTAssertTrue(result.message.contains("Calorie count cannot be nil."))

        // 7. Test with an empty title and zero calorieCount
        result = sut.updateCalorieItem(withId: validId, title: "", calorieCount: 0, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("The title cannot be empty."))
        XCTAssertTrue(result.message.contains("Calorie count must be greater than zero."))

        // 8. Test with a nil title and negative calorieCount
        result = sut.updateCalorieItem(withId: validId, title: nil, calorieCount: -100, calorieQuantity: 1, viewContext: mockContext)
        XCTAssertFalse(result.success)
        XCTAssertTrue(result.message.contains("The title cannot be nil."))
        XCTAssertTrue(result.message.contains("Calorie count must be greater than zero."))
